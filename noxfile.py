...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/salt_convert/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/salt_convert/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/salt_convert/*")
            ...
            #    "--include=src/saltext/salt_convert/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/salt_convert/config/schemas",
    )
